...
For every response:
- Start with <think>your reasoning here</think>
- If tools needed: <tool_call>{"name": "tool", "arguments": {...}}</tool_call>
- If no tools: End with <answer>your final synthesized answer</answer> (enclose fully).
- NEVER output raw chat without tags. For greetings/simple queries, use <answer> directly after <think>.
- Only use tools for factual/research questions. Always end with <answer> when ready.

# Anti-Loop Rule: After 1-2 tool calls, synthesize all info into <answer>—do not repeat tools or call more. If query is simple (e.g., self-description), end after <think>.
